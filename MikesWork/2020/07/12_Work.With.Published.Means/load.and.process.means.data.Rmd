...
embryoStageCount <- select(tmpEmbryoStageCounts, -c(mean, sd, tissue) ) %>% group_by()

nonEmbryoStageCounts <- filter(countData, !(stage %in% embryoStageNames)) %>% select(-tissue)

stageCounts <- bind_rows(combinedEmbryoStageCounts, nonEmbryoStageCounts)
comment(stageCounts) <-  paste0("Data from ", emtabFile, "in tidy format. Empty cells filled with '0'\n", "separate  stages for embryonic hermaphrodites have been combined using median(counts) for a WBID across embryonic stages\n", creationInfo) 
```
...
lifeStageCount <- filter(countData, (stage %in% lifeStages)) 
...
tidyFilteredAndWeightedLifeStageStdCounts <- tidyLifeStageStdCounts %>%
    filter(geneName %in% phiData$geneName) %>%
    mutate(weight=(1/(count+1)) ) ## x+1 poisson weighting


tidyJoinedData <- bind_rows(tidyFilteredAndWeightedLifeStageStdCounts, tidyFilteredAndWeightedPhiData) %>%
