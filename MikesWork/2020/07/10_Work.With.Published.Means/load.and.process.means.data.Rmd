...
save(tidyFullLifeStageData, mainLifeStages, tidyGeneIDLifeStageData, tidyWBIDLifeStageData, file = "processed.E-MTAB.data.Rdata")
```
...
fastaInfo <- tibble(info=names(seqData), gene=seqGeneNames, length=as.double(seqLength))
...
phiData  <- as_tibble(bind_cols(gene=seqGeneNames, phiPosteriorInfo)) #%>%
#    mutate_if(is.character, as.factor)  # convert char columns to factors
...
anomolousGenesNames <- (filter(fastaInfo, (length %% 3)!=0))$gene
anomolousGenes <- phiData %>% filter(gene %in% anomolousGenesNames)
...
filteredPhiData <- bind_cols(phiData, length=seqLength) %>% filter( !(gene %in% anomolousGenesNames) )
...
combinedPhiData  <- filteredPhiData %>% group_by(gene) %>% summarize(median_phi = median(phi), median_sd = median(sd), median_length = mean(length), median_logPhi=median(logPhi), median_logSd=median(logSd),  isoforms = length(phi) )
dim(combinedPhiData)
