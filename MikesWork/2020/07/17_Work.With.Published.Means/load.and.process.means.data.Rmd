...
- Use it this data to
	- Create a 'embryo Ce' stage for the analysis.
	Done using the mean across these various stages.
...
embryoStageCountMoments <- separateEmbryoStageCount %>%
    group_by(WBID) %>%
    summarise(mean=mean(count), inverseMean = 1/mean(count), median=median(count), sd=sd(count), logMean=log(mean), logSd = log(sd), logVar=2*log(sd), n=length(count), min(count), max(count), .groups="drop") %>% unique()
...
## From http://www.sthda.com/english/articles/32-r-graphics-essentials/131-plot-two-continuous-variables-scatter-graph-and-alternatives/
