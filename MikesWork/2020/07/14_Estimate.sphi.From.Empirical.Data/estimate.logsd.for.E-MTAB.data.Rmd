...
logLikCensoredFun(c(meanlog=1, sdlog=1, count=pull(tmpCount, count), threshold=pull(tmpCount, threshold)))

logLikCensoredFun(c(meanlog=1, sdlog=1, count=tmpCount[1, "count"], threshold=vector(tmpCount[1, "threshold"]))

## Demonstrate we can maximize function
maxLik(logLik=logLikCensoredFun, start=list(meanlog=mean(log(tmpCount$count), na.rm=TRUE),  sdlog=sd(log(tmpCount$count), na.rm=TRUE), count=tmpCount$count, threshold=tmpCount$threshold), method="BFGS", fixed=c("count", "threshold"))
