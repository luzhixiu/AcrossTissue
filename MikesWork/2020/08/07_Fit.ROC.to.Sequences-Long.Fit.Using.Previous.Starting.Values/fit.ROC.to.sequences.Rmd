...
- Previous code in 06_Fit.ROC....  *didn't work*.
There was a memory error that may be related to trying to use a restart file to start the run.  See 'r.session.crash.txt' in the 06 folder for more details
- Found bug where "Posterior" was used in R code rather than "Mean" as updated by me sometime ago.
- Currently using git repo commit: d856343a86b916e
# ESS Commands
...
myCreateGenomeObject <- function(fasta.file, gene.index = NULL){
    ...
    if( length(fasta.file) != length(gene.index) )
        stop('Error: Number of lists in fasta.file argument != number of lists in gene.index argument')
...
samples <- c(500,500)  ## steps determined by samples * thin
...
# while(round <= max.round){    
    ...
    output.restart.file <- paste0(output.dir,"Restart/tmp-rstart.round.", round, ".rst")
...
## Strangely a '0' appears at the end of the output when running the code below
parameter <-
