...
demingRegression <- function(directory.1,directory.2,include.AA=c())
{
  
  sel.1 <- read.table(directory.1,sep=",",header=TRUE,stringsAsFactors=F)
  sel.2 <- read.table(directory.2,sep=",",header=TRUE,stringsAsFactors=F) 
  ...
  ## Bypass the xstd non-positive error using fitted std
  reg <- deming(Posterior.2 ~ Posterior+0,data=df)
...
plotDeta <-function(data,b1,b0,reg.ci=NULL,categories=c("X","Y"),ci = F,file="test.pdf",title="Regression",range.xy = NULL)
{
  ...
  p<-(p
      ...
      + new_scale_color()
      + geom_point(data=data[sig,],size=3)
...
output <- demingRegression("/home/lu/AcrossTissue/ROC_Runs/Run_L2L3_Combined/Selection/L1_Selection.csv","/home/lu/AcrossTissue/ROC_Runs/Run_L2L3_Combined/Selection/dauer_Selection.csv")
...
for (fname1 in fileList){
  for (fname2 in fileList){
    ...
    if ((LS1=="dauer") & (LS2=="L1")){next}
    if ((LS1=="L1") & (LS2=="dauer")){next}
    
    print(LS1)
    print(LS2)
    output <- demingRegression(fDir1,fDir2)
    outputName=paste0("/",LS1," vs ",LS2,".pdf")
    outputName
    plot <- plotDeta(output$df,b1=unname(output$Slope),b0 = unname(output$Intercept),reg.ci=output$Slope.CI,categories = c(LS1,LS2),ci=T,title =expression(atop("Comparison of Selection "*Delta*eta)),file=paste0(target.directory,outputName),range.xy=NULL)
  }
}
