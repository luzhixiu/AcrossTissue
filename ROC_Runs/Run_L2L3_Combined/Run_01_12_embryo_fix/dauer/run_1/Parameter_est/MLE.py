...
deltaEtaFile="dauer_Selection.csv"
deltaMFile="dauer_Mutation.csv"
...
import sys
sys.path.insert(0,'/home/lu/AcrossTissue/Code')


from Bio import SeqIO
records=SeqIO.parse("dauer.fasta", "fasta")

MLE_Phi_List=[]
for record in records:
    seq=(record.seq)
    MLE_Phi=calPhiForGene(seq)
    MLE_Phi_List.append(MLE_Phi)
...
for line in lines:
    splitList=line.split(",")
    ROC_Phi=float(splitList[0])
    ROC_Phi_List.append(ROC_Phi)
    
import matplotlib.pyplot as plt
plt.style.use('seaborn-whitegrid')
import numpy as np

plt.plot(MLE_Phi_List, ROC_Phi_List, 'o', color='black');
    
from scipy import stats
print(stats.pearsonr(MLE_Phi_List, ROC_Phi_List))
print(stats.spearmanr(MLE_Phi_List, ROC_Phi_List))
...
from scipy import stats
plt.plot(log1p(MLE_Phi_List), log1p(ROC_Phi_List), 'o', color='black');
print(stats.pearsonr(log1p(MLE_Phi_List), log1p(ROC_Phi_List)))
print(stats.spearmanr(log1p(MLE_Phi_List), log1p(ROC_Phi_List)))
